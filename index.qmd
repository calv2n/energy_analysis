...
This comparison between XGBoost and AutoRegression in future predictions of power consumption in the Eastern United States is motivated by curiosity about the relative performance of these two models given non-linear relationships and interactions between features. 
...
With the up-to-date approach, both models have access to the true $y$ values up to $y_{t-1}$ when predicting $y_t$. This is a more realistic scenario when predictions are made one step at a time with updated information. We can see here that XGBoost really shines. While the AR model performs well, XGBoost slightly outperforms it.
...
Interestingly: we see a more comparable story between the two models in this setting. However, the XGBoost model still outperforms the AR model.

## Conclusion

In this project, we compared the performance of XGBoost and AR models on a time series dataset of PJME hourly electricity usage. We found that XGBoost outperforms AR models in both the rolling and up-to-date settings, with the up-to-date setting showing a more comparable story between the two models. 

Overall, XGBoost provides a more accurate and robust prediction method for this time series data, likely due to its ability to capture non-linear relationships and the interactions between time points. 
